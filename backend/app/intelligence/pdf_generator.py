...
_PKG_FONTS = Path(__file__).resolve().parent / "fonts"
_FONT_CANDIDATES = [
    (
        "LiberationSans",
        [
            "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf",
            str(_PKG_FONTS / "DejaVuSans.ttf"),
            "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
        ],
        [
            "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf",
            str(_PKG_FONTS / "DejaVuSans-Bold.ttf"),
...
def _register_fonts():
    ...
    if _FONTS_REGISTERED:
        return
    try:
        for family, regulars, bolds, italics, bis in _FONT_CANDIDATES:
            regular = _first_existing(regulars)
            bold = _first_existing(bolds) or regular
            italic = _first_existing(italics) or regular
            bi = _first_existing(bis) or bold
            if not regular:
                continue
            ...
            pdfmetrics.registerFont(TTFont(f"{family}-BoldItalic", bi))
            pdfmetrics.registerFont(TTFont(f"{family}-Oblique", italic))
            # Also register as Hebrew aliases used elsewhere
            pdfmetrics.registerFont(TTFont("Hebrew", regular))
            pdfmetrics.registerFont(TTFont("Hebrew-Bold", bold))
            _FONTS_REGISTERED = True
            logger.info("PDF fonts registered from %s", regular)
            return
        logger.warning("No TTF fonts found for PDF Hebrew/Latin support")
        _FONTS_REGISTERED = True
    except Exception as e:
        logger.warning(f"Could not register PDF fonts: {e}. Hebrew text may render as blanks.")
